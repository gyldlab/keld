...
| Cold start → first paint | ≤ 300 ms | 1–3 s | macOS **over budget** (KEL-64 untraced-arm oracle; not gyldlab/keld#10 `PageLoadEvent::Finished`); Windows **472 ms** (2026-08-15 session, direct-COM backend) — ~1.6x over, floor is Chromium boot in controller creation |
...
**First paint:** KEL-64 external double-rAF beacon on **untraced** arms. Do not treat RSS-ready, window-visible, or gyldlab/keld#10 `PageLoadEvent::Finished` as paint.
...
| Neutralino 6.9.0 (stock) | **0/11** | not reportable | not reportable | Fail-closed: canonical HTML was served, but its beacon reported `document.hasFocus() = false` despite target-app activation. Stock macOS Neutralino makes the window key without making its WKWebView first responder; this requires a runtime fix, not a synthetic click or relaxed oracle. |
| Neutralino 6.9.0 + local macOS focus patch | **11/11** | **595.975 / 625.097 ms** | **206,976 / 207,216 KiB** | Patched-runtime proof only, not a stock or upstream-release score. The one-call patch makes the `WKWebView` first responder after Neutralino's existing macOS bring-to-front/restore handling inside `window.focus()`. Every arm accepted the visible/focused double-rAF beacon and passed foreground continuity, restoration, and tail seal. |
...
`/tmp/keld-kel64-neutralino-focus-onecall-20260817.json`,
`/tmp/keld-kel64-neutralino-focus-repro-one-20260817.json`,
`/tmp/keld-kel64-final-electrobun-20260817.json`, and
`/tmp/keld-kel64-final-electron-valid-20260817.json`.

### KEL-64 macOS first paint and attribution (2026-08-17)

Paint oracle: independent double-rAF image beacon on a focused document, clock
armed before spawn. **Do not** treat [gyldlab/keld#10](https://github.com/gyldlab/keld/pull/10)
`PageLoadEvent::Finished` as first paint (navigation completion; clock starts
after process launch).

Reported scores come only from **trace-disabled** arms (AC4
[`c3030d5`](https://github.com/gyldlab/keld-benches/commit/c3030d579b606ac00d3ba9e0fba8f57987a41baa)).
The untraced interleaved Keld row above (median **342.911 ms** @
[`9e7c83d`](https://github.com/gyldlab/keld-benches/commit/9e7c83d1a5c94a790b2e3ed0a89855e3aed4ab9b))
is that class of number. Traced-arm construction is attribution, not a paint
score.

Fixture [`macos/keld/hello`](https://github.com/gyldlab/keld-benches/tree/aae2e12f998ff47805eed38083c624525d87b9a8/macos/keld/hello):
...
Record: [`kel64-startup-attribution.json`](https://github.com/gyldlab/keld-benches/blob/aae2e12f998ff47805eed38083c624525d87b9a8/macos/keld/hello/kel64-startup-attribution.json)
(Keld `59e0987`, recipe
[`258756c`](https://github.com/gyldlab/keld-benches/commit/258756c051fb951590d69d16fbad96f85c605d8b);
n=11 traced diagnostic, **not** a published score).

| Attribution (traced; not a score) | Median / p90 | Bounds |
|---|---:|---|
...
Construction does not explain the p90 tail. Limitation
`external_webkit_scheduling`. No product `keld-wv` window-build optimisation
from this slice.
...
| First paint ≤ 300 ms | **Windows: 472 ms (2026-08-15), ~1.6x over.** macOS: KEL-64 untraced-arm oracle is over budget (interleaved median 342.911 ms); construction ~149 ms vs beacon ~352 ms is residual WebKit, not a `keld-wv` build target. Do not use gyldlab/keld#10 `PageLoadEvent::Finished`. Linux: still not instrumented. |
...
| 5 | Cold start first paint (≤300 ms) | **can win with work** — **budget missed on both OS** | Windows (2026-08-15): Keld **472 ms** vs Tauri 483/506; Electron 278; floor is Chromium boot inside `CreateCoreWebView2Controller`. macOS: KEL-64 **untraced-arm** oracle (not #10 PageLoad Finished); construction ~149 ms, beacon ~352 ms, residual is WebKit (`external_webkit_scheduling`). |
...
fake `bench/` CI; treat traced-arm construction as a paint score.
