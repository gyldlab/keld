...
This file is the detailed engineering ledger. For a quick product-level reading,
the current **initial** evidence says:
...
| KELD vs Electron main-process RSS, Windows direct-COM session | **19,552 KB vs 89,140 KB** | KELD's native host process used about **4.6× less RSS**. This does **not** mean lower total application RSS; Electron's smaller process tree won total RSS in the broader Windows measurements. |
| KELD vs Tauri host executable, Windows | **484,864 B vs 8,634,880 B** | KELD's current direct-COM host is about **17.8× smaller** in the host-executable lane. This is not an installer comparison. |
...
lead. Do not turn them into an overall framework score.

### Plain-language metric guide

| Metric | What it tells a user |
|---|---|
| **Host working set / main-process RSS** | How much physical memory the framework's native host process is keeping resident while the app is running. Lower can leave more RAM for the user's actual application. |
| **Total process-tree RSS** | Memory across the whole owned process tree. This is broader than host RSS and can tell a different story. |
| **Host executable size** | The size of the native framework binary itself. It is not the same as download or installer size. |
| **First paint** | Time from launch until the first rendered frame is observable. Lower generally means the app feels visible sooner. |
| **IPC RTT** | Time for one message to travel to another KELD process and back. It describes framework communication overhead, not whole-app startup. |
...
[`91fd3e66b402cb9e4ba79133dc68a270011444c6`](https://github.com/gyldlab/keld-benches/commit/91fd3e66b402cb9e4ba79133dc68a270011444c6): Wails
...
|---|---:|---|
| **Keld** `39be9cc` direct COM | **469 ms** | [`windows-first-paint-kel65-direct-com.json`](https://github.com/gyldlab/keld-benches/blob/686d1ab632f023488227fcb5e7b44009df899653/windows/bench/windows-first-paint-kel65-direct-com.json) @ [`686d1ab`](https://github.com/gyldlab/keld-benches/commit/686d1ab632f023488227fcb5e7b44009df899653); also `MEASUREMENTS.md` |
| Tauri 2.11.5 (same session) | 479 ms | same files |
| Electron 43.4.0 (same session) | 275 ms | same files |
| Keld wry baseline `137633f` | 467 ms | [`windows-first-paint-kel65-baseline.json`](https://github.com/gyldlab/keld-benches/blob/686d1ab632f023488227fcb5e7b44009df899653/windows/bench/windows-first-paint-kel65-baseline.json) |
...
metric. Historical Windows sessions (906 ms ad-hoc, 1,289 ms, 573 ms) are superseded; see
...
### Coalition RSS (KEL-64, host + WebKit helpers) — non-publishable diagnostics

Different contract from host-only `ps`. Not comparable to 72.6–77.8 MiB.
Not paint. These rows are explicitly **non-publishable diagnostics** because
their raw JSON lived only under `/tmp` and cannot satisfy the immutable
SHA-plus-path rule.
...
seed, the same canonical payload, the same sha-verified `keld.exe`
(`b7f1e703…`), the same fixture tree, AC power — plus two shorter sessions. Across
all four:

| quantity | 2026-08-25 | 2026-08-26 (30r) | cross-session |
|---|---:|---:|---|
| **Keld host WS** — the scored value | 22,788 KiB | 22,754 KiB | **1.00×** |
| **Tauri host WS** — the baseline | 26,856 KiB | 26,848 KiB | **1.00×** |
| `conhost.exe` | 10,146 KiB | 10,144 KiB | **1.00×** |
| Bun child **private bytes** | 89,016 KiB | 88,916 KiB | **1.00×** |
| Bun child **working set** | 50,828 KiB | 22,968 KiB | **2.22×** |
...
|---|---:|---:|
| host | 22,788 KiB (reproduces) | 3,920 KiB (reproduces) |
| supervised Bun child | 50,828 KiB → 22,968 KiB (**2.22×**) | 89,016 KiB (reproduces) |
| `conhost.exe` | 10,146 KiB (reproduces) | not recorded |

Each sum above is the median of the per-run totals, which is what the harness
emits as `keld_processes_ws_kib`. It differs by ~10 KiB from adding the working-set
column, because a median of sums is not a sum of medians.

**What was ruled out.** The binary is byte-identical to the one the published
document cites; `windows/keld/hello/` is unchanged since that commit and carries
no `node_modules`; the canonical payload makes no difference (22,972 KiB with it,
22,896 KiB without); and the Bun child's working set is flat within a run —
tracked once per second for 20 s it moved 22,868 → 22,928 KiB, so this is not a
...
`windows_subsystem = "windows"` for release builds, so it has none — 0 KiB in
...
- Windows first-paint JSON @ [`686d1ab`](https://github.com/gyldlab/keld-benches/commit/686d1ab632f023488227fcb5e7b44009df899653) `windows/bench/`
...
`requestAnimationFrame`, as a paint-opportunity proxy; it does not establish
compositor completion or display scanout. A
...
[`windows/bench/Measure-FirstPaint.ps1`](https://github.com/gyldlab/keld-benches/blob/e3f65f42aa550ed8d6ccfdf0d5c8cad74b85704f/windows/bench/Measure-FirstPaint.ps1)
at [`gyldlab/keld-benches@e3f65f42aa550ed8d6ccfdf0d5c8cad74b85704f`](https://github.com/gyldlab/keld-benches/commit/e3f65f42aa550ed8d6ccfdf0d5c8cad74b85704f);
raw samples (git SHAs, exe SHA-256, versions, exact command) in
[`windows/bench/windows-first-paint.json`](https://github.com/gyldlab/keld-benches/blob/e3f65f42aa550ed8d6ccfdf0d5c8cad74b85704f/windows/bench/windows-first-paint.json).
Median of 5, same machine/session.

> **Evidence caveat:** that generic path was later overwritten by the
> 2026-08-15 `f28d696` median-of-7 session. The table below is derivable from the
> immutable `e3f65f4` blob linked above, not from the file at `main`'s tip. New
> result files are session-named and immutable because of this overwrite.
...
## Windows first paint after the controller-bounds fix (2026-08-15, median of 7)
...
|---|---|---|---|---|
| **Electron** 43.4.0 | **372 ms** | 91,988 KB | 224,252 KB | **4** |
| **Keld** `f28d696` | **573 ms** | **23,068 KB** | 351,472 KB | 7 |
| Tauri 2.11.5 | 589 ms | 28,256 KB | 352,316 KB | 7 |

Corrected from the committed raw after the PowerShell median-index defect was
found; the shared derivation check is in
[`gyldlab/keld-benches#10`](https://github.com/gyldlab/keld-benches/pull/10).
...
| Claim | Now |
|---|---|
| "Keld first-paints 1.87x slower than Tauri" | **Retired.** 573 vs 589 ms is numerically close in this session, with no demonstrated lead because the evidence has no paired uncertainty analysis. The published gap was the bounds defect plus harness spawn overhead, both fixed. |
| Keld starts fast | **Near parity with Tauri in this session, not leadership.** Electron still first-paints ~1.5x faster than both WebView2 arms. |
| Historical relation to the ≤ 300 ms target | Keld **573 ms**, Tauri **589 ms**, Electron **372 ms**. The remaining Keld cost is `WebViewBuilder::build` (~550 ms: WebView2 environment + controller creation), which is engine-inherent on this path. Measurement only; no current pass/fail label. Tracked on KEL-62. |
| Keld has the lowest main-process RSS | **Yes** — unchanged (23,068 KB this session; absolutes drift with machine warmth, ordering does not). |
...
|---|---|---|
| Electron 43.4.0 | 275 ms | 286 ms |
| **Keld** | **469 ms** | **467 ms** |
| Tauri 2.11.5 | 479 ms | 490 ms |

Both runs: [`windows/bench/`](https://github.com/gyldlab/keld-benches/tree/686d1ab632f023488227fcb5e7b44009df899653/windows/bench)
@ `686d1ab` (raw samples `windows-first-paint-kel65-*.json`). Across the two
A/B runs, each arm sits 86–110 ms below the earlier 2026-08-15 `f28d696`
session — session conditions, which is why absolutes are never compared across
sessions. Corrected raw derivation: [`gyldlab/keld-benches#10`](https://github.com/gyldlab/keld-benches/pull/10).

What the rewrite *did* measurably change:

| Metric | wry backend | direct COM |
|---|---|---|
| `keld-host.exe` (release) | 625,152 B | **484,864 B** (−22.4%) |
...
| SmartScreen comparison | — | ON 469 ms vs OFF 453 ms: **inconclusive** without a confidence interval |
...
| Keld vs Tauri | **Keld led in both same-session runs** (469 vs 479; 467 vs 490). Margin 10–23 ms is small against noise — claim "consistently ahead this session", not a ratio. |
| Keld ships SmartScreen-disabled (inherited wry default) | **Fixed.** No browser args, verified on the live process. The one-session 469 ms ON vs 453 ms OFF comparison is inconclusive and does not establish a startup cost. |
