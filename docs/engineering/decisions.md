...
new `_with_ready` variants in each `keld-wv` backend is still open, and no interactive
GUI run of the new concurrent `keld dev` flow (real window + real Bun session
end-to-end) has been done yet — only the isolated unit/integration tests above.
