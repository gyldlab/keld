...
now `keld_ipc::guard_dispatch::dispatch_privileged` (KEL-69) call it. No production
capability uses `dispatch_privileged` yet — `keld-native`'s modules are still names only
(no `fs.read`/`fs.write` implementation; that is KEL-71). `keld dev`'s app-process spawn
...
**Next.** Host-owned scoped `fs.read`/`fs.write` (KEL-71, no longer blocked) — MUST call
`dispatch_privileged` rather than reimplement the check. Keep shipping Linear Phase 2
(window + kipc echo + crate map) on the four uniques. Do not add a fifth unique to look
complete.
