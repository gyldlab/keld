...
| C2 | **Verification gate**: AGENTS.md says `cargo test --workspace`; CI/justfile use `cargo nextest run --workspace --profile ci`. | `AGENTS.md` § Verification floor; `.github/workflows/ci.yml`; `justfile` | Update AGENTS.md + `docs/agents/workflow.md` to canonical nextest command; keep `cargo test` as fallback note. |
