...
crashes, because the supervisor publishes stdout and its `Exited` event *before* it
records the crash: a host that samples the count when it notices the ready marker
can already see a death that happened after the app was live, and would forgive it.
The ledger therefore also records how long stdout was at the moment of each crash,
and the session compares the ready marker's offset against it — answering "printed,
then crashed" versus "crashed, then printed" from recorded facts rather than from
