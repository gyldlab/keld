...
Windows release startup selects its persistent WebView2 profile before creating a
listener, Bun child, or window. `keld-core` accepts one Authenticode signature with no
secondary signatures. It derives publisher scope from the verified leaf certificate's
SPKI and reads the
...
Keld creates Windows profile directories atomically with a protected inheritable DACL
for the current user, SYSTEM, and Administrators. Subsequent validation rejects foreign
ordinary-user read, write, execute, delete, or ACL/owner authority while retaining the
AppContainer and capability ACEs WebView2 requires.
