...
Under the [approved KEL-135 contract](../specs/kel135-persistent-profile-identity.md),
macOS release startup validates the running code against an Apple-issued
developer signing requirement before reading its Team Identifier and code-signing
identifier. The host derives one profile identity and deterministic UUID, durably binds
that UUID in both directions to the full identity, and gives one selected
identifier-addressed `WKWebsiteDataStore` to every view in the engine. Apple owns the
physical website-store location; Keld reports only its metadata-root controls and the
WebKit identifier/persistence read-back. Persistent mode requires macOS 14 or later and
fails closed below it. Explicit dev mode uses one memory-only nonpersistent store per
launch, including on earlier macOS where `WKWebsiteDataStore.identifier` is unavailable.
One package/profile lease excludes a second same-app host until the host owner exits.
After a non-idle host crash, normal persistent startup remains quarantined for that boot;
purge is an exact-identity operation that waits for WebKit's removal completion and a
follow-up identifier enumeration before it clears binding records. Real second-user,
older-macOS, reboot-recovery, and saved-media-grant acceptance are recorded separately;
none follows from these implementation contracts alone.
...
for the corresponding restart/revocation proof. The adopted macOS evidence contract
in KEL-135 §7.3 exercises public site authority across clean restart, with native
prompt and capture controls. Its stable `macos-media-saved-grant` artifact row requires
`macos_media_contract=public-grant-restart-v1`; it does not claim that WKWebsiteDataStore
persisted or deleted a camera/microphone permission record. Acceptance remains separate
from this contract and from KEL-102's verified session-policy integration.
