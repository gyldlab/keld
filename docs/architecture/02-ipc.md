...
- **v0 host lifecycle (KEL-72):** `LIFECYCLE_CHANNEL` is `ChannelId(3)`. The host
  ...
  handle does not wake a blocking local `read` (rust-lang/rust#121594). This is
...
- Host never blocks on either peer; every await point has a deadline. v0 app-link
  ...
  `read_frame` still cannot retry after Timeout. The readiness-driven reader
  (and credit windows) remain later work.
