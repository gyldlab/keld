...
- **Path and URL scopes** share one vetted-pattern matcher in the host (no regex
  ...
  *A grant must name a destination:* if stripping `/**` leaves only a scheme and
  ...
  Consequences worth stating rather than leaving to be discovered. A colon starts
  a scheme only at the very start of the grant (RFC 3986 §3.1), so
  `$APPDATA/cache/https:/**`, `/srv/backup:/**` and the `{BS}{BS}?{BS}C:/**` shape
  `fs::canonicalize` returns on Windows all stay ordinary path globs. A bare `X:`
  is a drive root, so `C:/**` covers the drive; that exemption stops at the colon,
  so `a://**` is refused like any other scheme glob — but `C:/**` and `a:/**` are
  the same string, so a one-letter scheme's `X:/**` is a path glob and is the one
  shape the guarantee above does not cover. A *relative* grant whose first segment
  ends in a colon (`cache:/**`) is read as a scheme and is inert; write
  `./cache:/**` or use a `$VAR`. A schemeless protocol-relative resource
  (`//host/x`) has no scheme to anchor to and is matched as a path, so `//**` is a
  path glob and carries no authority guarantee. The separator is required
  literally, so a grant ending at an origin does not cover that origin written
  with a port, query or fragment (`https://api.myapp.com:443/v1`, `…?q=1`,
  `…#f`): spell the destination the way the host will send it, port included. And
  a grant that names no destination is inert rather than rejected — it loads, then
  authorises nothing, and no deny text or `keld doctor` check says so yet.
