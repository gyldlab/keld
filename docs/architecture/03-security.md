...
- **Path and URL scopes** share one vetted-pattern matcher in the host (no regex
  ...
  *A grant must name a destination:* if stripping `/**` leaves only a scheme and
  ...
  longer sibling `api.myapp.com.evil.test`. **There is deliberately no glob that
  grants "any host"** — name the origins. `**` is the only wildcard a path or URL
  scope has; a lone `*` matches literally. (Channel grants are a separate,
  not-yet-live vocabulary — see below.)
  Four consequences worth stating rather than leaving to be discovered. A single
  character before the first colon is a Windows drive, not a scheme, so `C:/**`
  stays an ordinary path glob — and a one-letter scheme therefore cannot express a
  URL grant. A schemeless protocol-relative resource (`//host/x`) has no scheme to
  anchor to and is matched as a path, so a `//**` grant is a path glob and does
  not carry the guarantee above. The separator is required literally, so a grant
  ending at an origin does not cover that origin written with a port, query or
  fragment (`https://api.myapp.com:443/v1`, `…?q=1`, `…#f`): spell the destination
  the way the host will send it, port included. And a grant that names no
  destination is inert rather than rejected — it loads, and then authorises
  nothing.
