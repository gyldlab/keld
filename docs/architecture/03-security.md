...
- **Path and URL scopes** share one vetted-pattern matcher in the host (no regex
  ...
  *A grant must name a destination:* if stripping `/**` leaves a scheme followed
  ...
  `"https:///**"`, `"file:///**"`, `"https:<TAB>//**"` — the grant matches nothing.
  ...
  strips tab/LF/CR before parsing at all. The rule takes any ASCII whitespace,
  which is wider than that and only ever refuses more. *A grant must not reach past the authority it does name:* for a
  ...
  scheme glob — but `C:/**` and `a:/**` are the same string, so a one-letter
