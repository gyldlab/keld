...
- **Path and URL scopes** share one vetted-pattern matcher in the host (no regex
  ...
  *A grant must name a destination:* if stripping `/**` leaves only a scheme and
  separators — `"https://**"`, `"https:/**"`, `"https:///**"`, `"file:///**"` —
  the grant matches nothing. Those spellings are refused together because they are
  not interchangeable to a parser: `https:/host` carries no `//`, so RFC 3986 reads
  a path, yet WHATWG URL parsing — a browser, `fetch`, a webview — still reads the
  host out of it. *A grant must not reach past the authority it does name:* for a
  ...
  scheme's `X:/**` is a path glob, and it is the one shape the guarantee above
  does not cover. The doubled-separator drive spellings `C://**` and `C:///**` do
  carry separators and are therefore inert; write `C:/**`. **A relative grant
  whose first segment ends in a colon** (`cache:/**`) is read as a scheme and is
  inert; write `./cache:/**` or use a `$VAR`. **A schemeless reference has no
