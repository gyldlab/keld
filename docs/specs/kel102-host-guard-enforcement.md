...
| `KEL-102-D5` | `keld_ipc::guard_dispatch::dispatch_privileged` is the sole production guard-before-handler boundary and sole production caller of `keld_guard::evaluate`. Before an FS decision it calls the guard-owned allocation-free `validate_fs_request`; direct `evaluate` retains literal matcher semantics for permission-explain tooling. Its `Allow(ScopePermit)` result lends `&ScopePermit` to the callback; the callback result cannot borrow it, and the permit has no public constructor. `keld-native` uses that borrow to select the first-match retained grant for registered native channels; other direct consumers ignore it. `keld-core` resolves trusted context, decodes, validates, and routes; it never evaluates, accepts a permit/index, or adds a second authorization check. |
