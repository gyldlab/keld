...
| 7 | Started-frame fixtures give the enclosing session/call deadline and `T + APP_LINK_IO_DEADLINE` distinct values in both orderings. Partial header and payload reads expire at the earlier absolute instant with exact `KELD-IPC-006`; ignoring either enclosing or started-frame deadline, or resetting either clock per read, fails its named monotonic-bound assertion. The exact `ubuntu-latest`, `macos-latest`, and `windows-latest` `check` rows are the acceptance fixture. |
| 8 | An idle accepted production transport crosses multiple bounded reader polls without starting a frame clock, then terminates at its separately declared session/call deadline with exact `KELD-IPC-006`; a separate shutdown case remains promptly cancellable. The fixture records a bounded read-attempt/cadence observable so removing the accepted-stream blocking reset or immediately retrying `WouldBlock` in a tight loop fails rather than passing as liveness. The exact three `check` rows are the acceptance fixture. |
