...
| `keld-guard` | [03-security](../architecture/03-security.md) | `parse_manifest` / `load_manifest` / `evaluate` for `app.<group>.<action>` path scopes; `Principal`, `Decision`, `DenyReason`; privileged kipc `FS_READ_CHANNEL` evaluates before the handler | `$VARS`/symlink canonicalization, channel grants, recorder, audit log, host `fs.read`/`fs.write` (KEL-71) |
