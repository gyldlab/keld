...
> named independent protocol/security evidence under the standing repository-owner delegation,
> listed under `## Review gates` in the PR (root [`AGENTS.md`](../../AGENTS.md)).
> `crates/keld-ipc/AGENTS.md` adds the mechanics: a change to
...
current echo adapter still carries hand-written TypeScript payload interfaces. KEL-98 proposes a
bounded bootstrap that, after approval, would derive compile-time TypeScript payload declarations
from the already-shipped Rust `EchoRequest` / `EchoResponse` structs to retire that mirror; it would
...
1. **The macOS/Windows dev compiler reads name + entry + renderer.** `keld dev`
...
On macOS and Windows, `keld dev` first spawns the staged no-flag host. The
macOS authenticated guardian or Windows primary owner composes
`keld_runtime::Supervisor`, which owns Bun's captured stdout/stderr and termination
ledger. The command factory sets argv/cwd/env for the staged entry:
...
|---|---|---|
| `KELD_APP_LINK` | `<endpoint>#<64 hex chars>` — Unix endpoint is the UDS path; Windows endpoint is `\\.\pipe\keld-<64 lowercase hex>`; both are minted by `keld_ipc::BootstrapListener` | The generated `main.ts`; absence is a hard error; missing `#token` is `KELD-IPC-007` |
...
speaking kipc itself via its embedded client, so `run_dev_echo` no longer takes a `keld_bin` parameter at
...
```ts
// generated src/main.ts (body source: templates/hello/src/main-body.ts)
...
Bun. `KELD_BIN` disappeared when the direct kipc client landed. `KELD_LINK`,
