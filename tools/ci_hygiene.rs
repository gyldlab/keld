...
const WINDOWS_MEDIA_ACCEPTANCE_COMMANDS: &[&str] = &[
    ...
    "$expectedProfilePath = [IO.Path]::GetFullPath((Join-Path ([IO.Path]::GetTempPath()) $expectedProfileLeaf))",
    ...
    "if ($expectedProfilePath.StartsWith($sharedProfileRoot, [StringComparison]::OrdinalIgnoreCase)) { throw 'Windows media fixture temp root overlaps the shared dev.keld profile root' }",
...
mod tests {
    ...
    fn windows_media_acceptance_commands_cannot_be_removed_or_made_inert() {
        for (needle, replacement) in [
            ...
            (
                "$expectedProfilePath.StartsWith($sharedProfileRoot, [StringComparison]::OrdinalIgnoreCase)",
                "$false",
            ),
            (
