...
const WINDOWS_MEDIA_ACCEPTANCE_COMMANDS: &[&str] = &[
    ...
    "if ($loggedProfilePath -cne $row.profile_path -or (Split-Path -Leaf $loggedProfilePath) -cne $expectedProfileLeaf) { throw \"Windows media row profile identity did not bind its process and nonce: $rowStem\" }",
    ...
    "if ($seenNonces.ContainsKey($receiptFields.nonce) -or [uint64]$receiptFields.nonce -eq 0) { throw \"Windows media receipt nonce was zero or reused: $rowStem\" }",
    ...
    "if (($row.mode -ceq 'force-allow' -and $row.outcome -cnotmatch $expectedOutcome) -or ($row.mode -cne 'force-allow' -and $row.outcome -cne $expectedOutcome) -or $receiptFields.runtime -cnotmatch '^[0-9]+(\\.[0-9]+)+$' -or $row.origin_uri -cnotmatch '^http://127\\.0\\.0\\.1:[0-9]+/$' -or $row.host_pid -le 0 -or $row.browser_pid -le 0 -or [int]$receiptFields.tid -le 0 -or [Convert]::ToUInt64($row.registration_identity, 16) -eq 0) { throw \"Windows media row lacked live runtime, process, origin, sender, or outcome evidence: $rowStem\" }",
    "}",
