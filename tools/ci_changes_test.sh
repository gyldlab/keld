...
expect_flags() {
    ...
    local expected="$2"
    local actual="$3"
    actual="$(grep -Ev '^packages=' <<<"$actual")"
...
expect_flags "runtime-only Rust change avoids host GUI smoke" "$runtime_flags" "$runtime_classification"
...
expect_flags "host dependency closure routes IPC change to GUI smoke" "$host_dependency" "$host_classification"
...
expect_flags "workflow input exercises all conditional lanes" "$all_true" "$workflow_classification"
