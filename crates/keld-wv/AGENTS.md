...
- Linux: agents MUST probe the GPU stack and apply safe-mode before init — MUST NOT instruct env-var exports. Emit `degraded-rendering`. Implemented: `webkitgtk::probe_gpu_stack` (NVIDIA+Wayland → `WEBKIT_DISABLE_DMABUF_RENDERER=1` on this process's own environment, before any GTK/WebKit call), `WebKitGtkEngine::gpu_safe_mode()` exposes the result for `keld doctor`.
