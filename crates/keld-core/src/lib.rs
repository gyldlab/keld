...
//!
//! Privileged kipc `Call` is checked by [`dispatch_privileged`] before the
//! handler runs (`docs/architecture/03-security.md` §1). Echo stays ungated.
...
pub use privileged::{
    FS_READ_CHANNEL, FS_READ_OPERATION, PrivilegedSession, deny_call_error, dispatch_privileged,
    serve_privileged_session,
};
